...
server <- function(input, output, session) {
  ...
  output$exampleplot <- renderPlotly({
    ...
    q <- wintalentdata %>%
      filter(School %in% c("Penn State", "USC", "Oregon State", "Army", "Boise State")) %>%
      ggplot() +
      ...
      geom_text(subset(wintalentdata, School %in% c("Penn State", "USC", "Oregon State", "Army", "Boise State") & Season == 2018),
  ...
  output$table <- DT::renderDataTable({
    datatable(wintalentdata,
              options = list(
                lengthMenu = c(5, 10, 15), # Number of rows to display per page
                pageLength = 5, # Initial number of rows to display per page
                ordering = TRUE, # Enable column sorting
                searching = TRUE, # Enable searching/filtering
                info = TRUE, # Show table information (e.g., number of rows)
                paging = TRUE # Enable pagination
              ),
              filter = "top", # Position of the search/filter input box
